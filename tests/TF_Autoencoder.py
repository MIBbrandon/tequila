...
compile_args = {'backend': 'qulacs', 'initial_values': inits,
                'samples': None}  # dict. allowed keys: backend, samples, noise, device, initial_values

tensorflowed = tq.ml.to_platform(h2_encoder, platform='tensorflow', compile_args=compile_args,
                                 input_vars=[input_variable])
# print(my_tf_encoder)

optim = optims.Adam(lr=0.01)

var_list_fn = lambda: tensorflowed.get_params_variable()  # We just want to train the parameters

loss_fn = lambda: tf.reduce_sum(tensorflowed())
...
print(tensorflowed.get_params_values())
